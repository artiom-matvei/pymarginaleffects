...
def get_one_variable_hi_lo(variable, value, newdata, comparison, eps, by, wts):
...
def sanitize_variables(variables, model, newdata, comparison, eps, by, wts):
    out = []
...
def sanitize_comparison(comparison, by, wts = None):
    ...
    if by is not False:
        if f"{comparison}avg" in estimands.keys():
            out = comparison + "avg"
    if wts is not None:
        if f"{comparison}wts" in estimands.keys():
            out = comparison + "wts"
        elif f"{comparison}avgwts" in estimands.keys():
            out = comparison + "avgwts"
    return out